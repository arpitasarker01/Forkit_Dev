"""
Lineage Graph — directed acyclic graph tracking provenance relationships
...
class LineageGraph:
    """
    In-memory directed acyclic graph of passport lineage.

    Backed by simple adjacency dicts — no external dependency.
    Can import/export to JSON for persistence via the registry.
    """

    def __init__(self):
        self._nodes: dict[str, LineageNode] = {}
        self._edges: list[LineageEdge] = {}
        self._adj: dict[str, list[str]] = {}   # source_id → [target_ids]
        self._radj: dict[str, list[str]] = {}  # target_id → [source_ids]
        self._edges: list[LineageEdge] = []
    ...
    # ------------------------------------------------------------------ #
    # Convenience constructors from passports
    # ------------------------------------------------------------------ #
    ...
    def register_model(self, passport_dict: dict[str, Any]) -> LineageNode:
        ...
        # Wire to parent model if fine-tuned
    ...
    def register_agent(self, passport_dict: dict[str, Any]) -> LineageNode:
        node = LineageNode(
            ...
            name=passport_dict["name"],
            version=passport_dict["version"],
            metadata={"role": passport_dict.get("role"), "creator": passport_dict.get("creator", {})},
        )
        self.add_node(node)
    ...
    def ancestors(self, node_id: str) -> list[LineageNode]:
        """All nodes reachable upstream (following edges outward)."""
        visited: set[str] = set()
        result: list[LineageNode] = []
        self._dfs(node_id, self._adj, visited, result)
        result = [n for n in result if n.id != node_id]
        return result

    def descendants(self, node_id: str) -> list[LineageNode]:
        """All nodes that depend on this node (reverse traversal)."""
        visited: set[str] = set()
        result: list[LineageNode] = []
    ...
    def summary(self) -> str:
        n_models = len(self.nodes_by_type(NodeType.MODEL))
        n_agents = len(self.nodes_by_type(NodeType.AGENT))
        return (
            f"LineageGraph — {len(self._nodes)} nodes "
            f"({n_models} models, {n_agents} agents), "
    ...
    def _would_create_cycle(self, source: str, target: str) -> bool:
        """DFS from target; if we reach source, adding this edge creates a cycle."""
        visited: set[str] = set()
        stack = [target]
        while stack:
            node = stack.pop()
